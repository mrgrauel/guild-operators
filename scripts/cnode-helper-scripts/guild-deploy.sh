...
download_cnodebins() {
  ...
  curl -m 200 -sfL https://github.com/IntersectMBO/cardano-node/releases/download/8.10.1-pre/cardano-node-8.10.0-linux.tar.gz -o cnode.tar.gz || err_exit " Could not download cardano-node release 8.10.0-pre from GitHub!"
  tar zxf cnode.tar.gz ./bin/cardano-node ./bin/cardano-cli ./bin/cardano-submit-api ./bin/bech32 &>/dev/null
  rm -f cnode.tar.gz
  [[ -f ./bin/cardano-node ]] || err_exit " cardano-node archive downloaded but binary (cardano-node) not found after extracting package!"
